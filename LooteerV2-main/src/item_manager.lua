...
function ItemManager.check_is_goblin_cache(item)
   return ItemManager.check_item_type(item, "goblin_cache")
end

function ItemManager.check_is_obol_bag(item)
   return ItemManager.check_item_type(item, "obol_bag")
end
...
if loot_manager.is_gold(item) or loot_manager.is_potion(item) then return false end
   if ItemManager.check_is_goblin_cache(item) and not settings.goblin_cache then return false end
   if ItemManager.check_is_obol_bag(item) and not settings.obols then return false end
   ...
   local is_obol_bag = settings.obols and ItemManager.check_is_obol_bag(item)
   ...
   -- Check greater affixes for magic (1-3) and rare (4) items
   if rarity >= 1 and rarity <= 3 and settings.magic_ga_count > 0 then
      local ga = Utils.get_greater_affix_count(item_info:get_display_name())
      if ga < settings.magic_ga_count then return false end
   end

   if rarity == 4 and settings.rare_ga_count > 0 then
      local ga = Utils.get_greater_affix_count(item_info:get_display_name())
      if ga < settings.rare_ga_count then return false end
   end
