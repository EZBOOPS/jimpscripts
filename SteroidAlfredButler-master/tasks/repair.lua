...
function extension.execute()
    ...
    console.print('[alfred:repair] execute - calling repair_all_items')
...
function extension.is_done()
    ...
    for _, item in pairs(items) do
        local dur = item:get_durability()
        console.print(string.format('[alfred:repair] is_done() item dur=%.1f', dur))
        if dur < 95 then
            is_done = false
        end
    end
    console.print('[alfred:repair] is_done() -> ' .. tostring(is_done))
    return is_done
end
